...
| 2026-05-25 | Fase 1 paso A — F-03 dual-write de credenciales (sin tocar frontend ni eliminar plaintext). Nuevo `stripe/credenciales.js` (AES-256-GCM con `CREDENTIALS_ENCRYPTION_KEY` desde Firebase Secrets); `inscripcionAlumno.js` escribe en `alumnoCredenciales/{uid}` además del plaintext en `alumnos`; nueva Function `getCredencialesAlumno` con Bearer auth; `firestore.rules` declara `alumnoCredenciales` con `allow get` para alumno+admin+directivo, `list/write` deny; script `scripts/migrar-credenciales.js` para backfill idempotente. El frontend sigue leyendo plaintext: cero cambio funcional para alumno/admin durante la coexistencia. | `d95b7a0` | (pendiente deploy con `CREDENTIALS_ENCRYPTION_KEY` configurada) |
| 2026-05-25 | Fase 1 paso B — F-01 nueva Function `verificarCertificadoPublico` (Admin SDK, rate-limited, comparación con `timingSafeEqual`, respuesta mínima). `certificadoService.verificarCertificado` migrado en el portal alumnos a llamar la Function; fallback al método legacy `getDocs(alumnos)` si la Function devuelve 404/5xx, para que la transición no rompa la verificación. Las reglas Firestore NO se tocan en este commit — el tightening final viene en otro paso después de validar la Function en producción. Procedimiento detallado en `docs/CERRAR_F01_LISTADO_ALUMNOS.md`. | `f90d0d0` | (pendiente deploy) |
| 2026-05-25 | Fase 1 paso C — F-02 `accessToken` por orden + F-05 log de email distinto. Nuevo helper `stripe/accessToken.js` con `generateAccessToken` (32 bytes), `hashAccessToken` (sha256), `validateAccessToken` (timingSafeEqual, expiración, modo legacy para órdenes pre-existentes sin `accessTokenHash`). `createPublicCheckout` genera token y lo incluye en `success_url` como `&t=…`; guarda `accessTokenHash` + `accessTokenExpiresAt` en la orden. `getInscripcionOrden`, `completeInscripcionParte1`, `completeInscripcionParte2`, `getInscripcionUploadUrl` exigen `accessToken` válido si la orden lo tiene (backward-compat para legacy). En `completeInscripcionParte1` se loguea sin bloquear cuando `emailContacto !== orden.cliente.email`. Frontend Gatsby (`inscripcion/completar.js`, `inscripcion/documentos.js`, `InscripcionParte1Form`, `InscripcionParte2Form`, `FileUploadField`) lee `?t=` de URL y lo reenvía. `inscripcionApi.js` acepta token opcional en todas las funciones. | `31989f0` + `6a3568f` | desplegado 2026-05-25 |
| 2026-05-25 | **Deploy en producción** — paso secuencial: (1) `CREDENTIALS_ENCRYPTION_KEY` v1 creado en Firebase Secrets (clave backup en `~/credentials-encryption-key.txt` 600 — Carlos respalda en gestor); (2) `firebase deploy --only functions:stripe` desde `alumnos-app/` — 2 funciones nuevas (`getCredencialesAlumno`, `verificarCertificadoPublico`) + 10 actualizadas; (3) smoke tests con curl: todos pasaron (400/401/404/200 según corresponda); (4) gap detectado en runtime — Gatsby antiguo no reenviaba `?t=`, riesgo de 403 en inscripciones nuevas; (5) `gatsby clean && npm run build && firebase deploy --only hosting` — bundle de producción incluye `params.get("t")`, gap cerrado; (6) `npm --prefix alumnos-app run build && firebase deploy --only hosting` (alumnos) — portal alumnos sirve `certificadoService` que ahora llama `verificarCertificadoPublico`; (7) `firebase deploy --only firestore:rules` — regla aditiva `alumnoCredenciales` desplegada. | — | **producción al día con código del repo** |
| pendiente | Fase 1 paso final — backfill `alumnoCredenciales` para alumnos pre-existentes (requiere `gcloud auth application-default login`) + migración del portal alumnos a `getCredencialesAlumno` + cleanup del plaintext en `alumnos`. Se agrupa con la migración del frontend; no bloquea operación porque el portal sigue leyendo plaintext. | — | — |
| 2026-05-25 | Pre-Live hardening — F-01 cerrado: `firestore.rules` cambió `allow list: if true` por `allow list: if canReadAdmin();` en `alumnos`. `verificarCertificadoPublico` valida que sigue funcionando (Admin SDK bypasea reglas). F-07 cerrado: `STRIPE_FUNCTIONS_ENV=production` activado en `.env.certificacionmontessori` (no commiteado, local); re-deploy stripe; verificado con curl que CORS rechaza `http://localhost:8000` y permite `https://certificacionmontessori.com`. **Bug crítico de redirección corregido**: `SITE_URL` apuntaba al canal antiguo `certificacionmontessori--stripe-test-sl45bkl4.web.app` que servía bundle viejo sin lectura de `?t=` — inscripciones nuevas habrían quedado rotas desde el deploy de F-02. Cambiado a `https://certificacionmontessori.com` + `ALUMNOS_SITE_URL` al dominio custom. Política de reembolsos publicada como `/reembolsos` con link en el footer; build + deploy. | (pendiente commit) | desplegado 2026-05-25 |
| pendiente | Migración Stripe Live — procedimiento detallado en `docs/CONFIGURAR_STRIPE_LIVE.md`. Pre-requisitos administrativos pendientes: KYC en dashboard Stripe (Carlos confirma "aún no he comenzado") + cuenta bancaria MXN (Carlos confirma "falta agregar"). Una vez listos: secrets Live + catálogo Live + webhook Live + smoke test con dinero real. | — | — |
