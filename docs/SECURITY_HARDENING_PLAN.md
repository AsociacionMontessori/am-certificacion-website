...
| pendiente | Tightening F-01 — cambiar `allow list: if true` a `allow list: if canReadAdmin();` después de 24-48h de validación de `verificarCertificadoPublico` en producción. Procedimiento en `docs/CERRAR_F01_LISTADO_ALUMNOS.md`. | — | — |
| pendiente | Migración Stripe Live — procedimiento detallado en `docs/CONFIGURAR_STRIPE_LIVE.md`. Requiere prerequisites administrativos (KYC, cuenta bancaria, política de reembolsos en el sitio). | — | — |
