...
| Site Scan | `EN_PROCESAMIENTO` | `2026-07-14` | `AMMAC / Codex` | Iniciado `Auditoria SEO completa - 2026-07-14` con alcance `Sitio web` desde la portada, limite de `100` paginas, sin subdominios, profundidad maxima `3`, velocidad `2` URLs/s y respetando `robots.txt`. Bing lo muestra en cola/en curso; paginas, errores y advertencias permanecen sin valor hasta que termine. No se inicia otro escaneo. Objetivo: cero Errors y clasificar cada Warning/Notice antes de cambiar codigo. |
...
| IndexNow Insights | `EN_PROCESAMIENTO` | `2026-07-14` | `AMMAC / Codex` | Tras el cambio global de titulos, el CLI valido contra el sitemap exactamente las `27` URLs canonicas y `api.indexnow.org` acepto el lote unico (`IndexNow accepted 27 URL(s)`). No se enviaron rutas privadas, transaccionales, redirigidas o externas. La opcion `IndexAhora` de la interfaz autenticada todavia muestra la introduccion publica y conduce a la guia de implementacion, aunque la [documentacion oficial](https://www.bing.com/webmasters/help/indexnow-0z209wby) sigue describiendo el informe Insights. No se genera otra clave, no se sustituye la verificacion ya desplegada y no se repite el envio. Se revisara una vez transcurridas `24-48` horas para comprobar si el informe aparece al procesarse el primer lote. Las transiciones publicas de WordPress se controlan por separado y los borradores nunca se notifican. |
