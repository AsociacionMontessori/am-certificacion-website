...
| Contrato de sitemap actual | `COMPLETADO_CON_EVIDENCIA` | `2026-07-23` | `AMMAC / Codex` | Consulta autenticada: `sitemap-index.xml` tiene estado `Correcto`, ultima lectura `2026-07-22`, `42` paginas descubiertas y `0` videos. Coincide con las `42` canonicas (`14` por idioma) verificadas por `npm run test:deployed-hosting -- https://certificacionmontessori.com`. No se volvio a presentar el sitemap. |
...
| Muestra manual de programas | `COMPLETADO_CON_EVIDENCIA` | `2026-07-23` | `AMMAC / Codex` | Inspeccion autenticada de Casa de Ninos en los tres idiomas. `/en/diplomados/casa-de-ninos/` ya esta en Google. Las versiones espanola y portuguesa figuran `Descubierta: actualmente sin indexar`, pero sus pruebas en vivo indican `La URL esta disponible para Google`, `La pagina se puede indexar` y un breadcrumb valido. No existe bloqueo tecnico y no se repitieron solicitudes de indexacion. |
| Page indexing | `EN_PROCESAMIENTO` | `2026-07-23` | `AMMAC / Codex` | El informe agregado sigue retrasado, con ultima actualizacion `2026-07-09`: `16` indexadas y `28` sin indexar. Motivos visibles: `9` paginas con redireccion, `3` alternativas con canonical adecuada, `9` rastreadas actualmente sin indexar y `7` descubiertas actualmente sin indexar. Sus `44` URLs conocidas no sustituyen el inventario de `42` canonicas del sitemap, porque incluyen redirecciones y variantes. |
...
| Contrato de sitemap actual | `EN_PROCESAMIENTO` | `2026-07-23` | `AMMAC / Codex` | Bing rastreo `sitemap-index.xml` el `2026-07-22` con estado `Success`, pero `sitemap-0.xml` conserva ultima lectura `2026-07-11` y `27` URLs. El resumen permanece en `28` por sumar el indice. Las URLs nuevas ya fueron descubiertas por IndexNow; se espera el refresco natural del sitemap hijo sin volver a enviarlo. |
...
| Muestra manual de programas | `COMPLETADO_CON_EVIDENCIA` | `2026-07-23` | `AMMAC / Codex` | `/diplomados/casa-de-ninos/`, `/en/diplomados/casa-de-ninos/` y `/pt-br/diplomados/casa-de-ninos/` fueron descubiertas el `2026-07-22` y aun figuran `Discovered but not crawled`. Las tres pruebas `Live URL` devolvieron `URL can be indexed by Bing`, dos tipos de marcado y el mismo aviso de `19` imagenes decorativas sin texto alternativo. El aviso coincide con el falso positivo ya documentado; no bloquea la indexacion. No se solicito indexacion de nuevo. |
