...
| IndexNow Insights | `EN_PROCESAMIENTO` | `2026-07-13` | `AMMAC / Codex` | Tras el cambio global de titulos, el CLI valido contra el sitemap exactamente las `27` URLs canonicas y `api.indexnow.org` acepto el lote unico (`IndexNow accepted 27 URL(s)`). No se enviaron rutas privadas, transaccionales, redirigidas o externas. Falta confirmar la recepcion en Insights; no se repetira el envio mientras se procesa. Las transiciones publicas de WordPress se controlan por separado y los borradores nunca se notifican. |
