...
const redirectPrefixes = (firebaseConfig.hosting?.redirects || [])
  ...
  .map(source => {
    ...
    const prefix = (wildcardIndex === -1 ? source : source.slice(0, wildcardIndex))
      .replace(/\/+$/, "")
    return prefix ? `${prefix}/` : "/"
  })

const isRedirectPath = pathname => redirectPrefixes.some(prefix => {
...
const normalizePublicPaths = paths => {
  ...
  for (const raw of paths || []) {
    if (typeof raw !== "string" || !raw.startsWith("/") || raw.startsWith("//") || raw.includes("\\")) continue
    const pathname = raw.split(/[?#]/, 1)[0]
    if (!pathname || !isSafePublicPath(pathname)) continue
