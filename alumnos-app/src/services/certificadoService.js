 ...
 * @param {string} [nivelId] - ID del nivel específico (opcional, para certificados/constancias por nivel)
 */
export const obtenerCertificado = async (alumnoId, nivelId) => {
  try {
    ...
    let graduacionCompleta = Boolean(graduacionDataRaw?.progresoCompleto);
    let nivelGraduacion = graduacionDataRaw?.nivelGraduacion || alumnoData.nivel || null;
    const programaGraduacion = graduacionDataRaw?.programaGraduacion || alumnoData.programa || null;
    let fechaGraduacion = ensureDate(graduacionDataRaw?.fechaGraduacion);
    let fechaIngresoNivelGraduacion = ensureDate(graduacionDataRaw?.fechaIngresoNivel);
    let fechaEgresoNivelGraduacion = ensureDate(graduacionDataRaw?.fechaEgresoNivel);
    let fechaIngresoActual = ensureDate(alumnoData.fechaIngreso);
    let fechaEgresoActual = ensureDate(alumnoData.fechaEgresoEstimada || alumnoData.fechaEgreso);
    let nivelActual = alumnoData.nivel || null;
    const estadoActual = alumnoData.estado || 'Activo';
    let estadoParaDocumento = graduacionCompleta ? 'Graduado' : estadoActual;

    // Obtener historial de niveles del alumno
    const historialNiveles = Array.isArray(alumnoData.niveles) ? alumnoData.niveles : [];

    // Si se especificó un nivel específico, buscar en el historial y sobreescribir datos
    let nivelEspecifico = null;
    if (nivelId) {
      ...
      if (nivelEspecifico) {
        ...
        if (esCompletado) {
          ...
          graduacionCompleta = false;
          fechaGraduacion = null;
          estadoParaDocumento = estadoActual;
        }
      }
    }
    ...
    const tieneNivelGraduacionDiferente = graduacionCompleta &&
      nivelGraduacion &&
      (alumnoData.nivel || null) &&
      nivelGraduacion !== alumnoData.nivel;
    ...
    return {
      ...
      codigoVerificacion,
      promedio,
      nivelEspecifico: nivelEspecifico ? {
        id: nivelEspecifico.id,
        nombre: nivelEspecifico.nombre,
        estado: nivelEspecifico.estado
      } : null,
      graduacion: {
