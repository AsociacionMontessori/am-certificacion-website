/**
 * Crea (idempotente) el producto + precio LIVE del Paquete Cósmico
 * (ebook_pack_cosmica_2, $400 MXN único) en la cuenta Stripe live.
 *
 * Uso:
 *   STRIPE_SECRET_KEY="$(cat /home/carlos/secrets/stripe_clave_secreta)" \
 *     node scripts/stripe-create-cosmica-price.cjs
 *
 * Imprime el price_... para pegarlo en STRIPE_PRICE_EBOOK_PACK_COSMICA.
 */

const path = require("path");

const SKU = "ebook_pack_cosmica_2";
const NAME = "Paquete Cósmico - Educación Cósmica + Guiones Cósmicos";
const AMOUNT_MXN = 400;
const CURRENCY = "mxn";
...
async function main() {
  ...
  // 2) Precio único de $400 MXN (reutiliza si ya existe uno activo igual).
  ...
  console.log(`\nSTRIPE_PRICE_EBOOK_PACK_COSMICA=${price.id}`);
}
